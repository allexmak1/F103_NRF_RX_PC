...
/***************************************************************************/
    ...
    // Это простой приемник с Enhanced ShockBurst:
    //   - RX address: 'ESB'
    //   - payload: 10 bytes
    ...
    // Передатчик отправляет 10-байтовые пакеты по адресу 'ESB' с Auto-ACK (включен ShockBurst).
...
//#pragma clang diagnostic ignored "-Wmissing-noreturn"
    while (1) {
         ...
         // Постоянно опрашиваем статус RX FIFO и получаем полезную нагрузку, если FIFO не пуст
         //
         // Это далеко не лучшее решение, но его можно использовать для тестирования
         // Более разумный способ - использовать пин IRQ :)
         // 
        if (nRF24_GetStatus_RXFIFO() != nRF24_STATUS_RXFIFO_EMPTY) {
            // Получите полезную нагрузку от трансивера
            pipe = nRF24_ReadPayload(nRF24_payload, &payload_length);

            // Очистить все ожидающие IRQ флаги
            nRF24_ClearIRQFlags();

            // Распечатать содержимое полезной нагрузки в UART
            UART_SendStr("RCV PIPE#");
            UART_SendInt(pipe);
            UART_SendStr(" PAYLOAD:>");
            Toggle_LED();
