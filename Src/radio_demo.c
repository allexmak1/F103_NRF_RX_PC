...
/***************************************************************************/
	...
	// Это простой передатчик с Enhanced ShockBurst (на один логический адрес):
	//   - TX address: 'ESB'
	//   - payload: 10 bytes
	//   - RF channel: 40 (2440MHz)
	//   - data rate: 2Mbps
...
//#pragma clang diagnostic ignored "-Wmissing-noreturn"
    while (1) {
    	// Подготовить пакет данных
    	for (i = 0; i < payload_length; i++) {
    		nRF24_payload[i] = (uint8_t) j++;
    		if (j > 0x000000FF) j = 0;
    	}

    	// Распечатать полезную нагрузку
    	UART_SendStr("PAYLOAD:>");
    	UART_SendBufHex((char *)nRF24_payload, payload_length);
    	UART_SendStr("< ... TX: ");
		...
		otx_plos_cnt = (otx & nRF24_MASK_PLOS_CNT) >> 4; // счетчик потерянных пакетов
		otx_arc_cnt  = (otx & nRF24_MASK_ARC_CNT); // счетчик автоматических повторных передач
    	switch (tx_res) {
			case nRF24_TX_SUCCESS:
				UART_SendStr("OK");
				break;
